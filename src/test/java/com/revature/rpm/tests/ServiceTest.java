...
public class ServiceTest {
  @Mock NotificationRepository mockNotificationRepository;
  ...
  public void readHappyPath() throws Exception {
    ...
    assertSame(
        "The Notification Repository returns true on success with setReadToTrue",
        true,
        returnValue);
  ...
  public void readFirstForbiddenPath() throws Exception {
    ...
    Boolean returnValue = notificationService.updateUnreadToRead(1, readDTO);
    fail(
        "Exception should have been thrown due to readDTO user not matching user listed on returned notification");
  }
  ...
  public void getAllIsReadFalse() {
    ...
    when(mockNotificationRepository.getNotificationsByUserIdAndIsReadFalseOrderByDateCreatedDesc(1))
        .thenReturn(mockList);
    ...
    for (int i = 0; i < returnedList.size(); i++) {
      ...
      assertSame(
          "At index " + i + "the is_read should be false, result is ",
          returnedList.get(i).isRead(),
          false);
      // verify the order is corrected
      if (i != 0) {
        if (returnedList.get(i - 1).getDateCreated().compareTo(returnedList.get(i).getDateCreated())
            < 0) {
  ...
  public void testPaginationFullPage() {
    ...
    Mockito.when(
            mockNotificationRepository.findByUserIdOrderByDateCreatedDesc(
                1, mockNote.getPageable()))
        .thenReturn(mockNote);
    assertThat(
            mockNotificationRepository
                .findByUserIdOrderByDateCreatedDesc(1, mockNote.getPageable())
                .getSize())
        .isGreaterThan(0);
    assertThat(
            mockNotificationRepository
